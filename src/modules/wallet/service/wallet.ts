...
export class WalletService {
      ...
      await (async () => {
        ...
        const primeSdk = new PrimeSdk(
          ...
          },
          {
            ...
            projectKey: "",
            rpcProviderUrl:
              process.env.CHAIN_PROVIDER ||
              "https://sepolia-bundler.etherspot.io/",
          },
        );
  ...
  async transfer2(
    ...
    if (parseFloat(balance) < parseFloat(value)) {
      console.log(
        "balance is not enough, balance: " + balance + " value: " + value,
      );
      return {
        status: 400,
  ...
  async transfer(
    ...
    const provider = new ethers.JsonRpcProvider(
      process.env.CHAIN_PROVIDER || "https://sepolia-bundler.etherspot.io/",
    );
    ...
    const transactionData = erc20Instance.interface.encodeFunctionData(
      "transfer",
      [recipient, ethers.parseUnits(value, decimals)],
    );
    await primeSdk.clearUserOpsFromBatch();
  ...
  async getBalance(certificate: string): Promise<Response | undefined> {
    try {
      ...
      const erc20Instance = new ethers.Contract(
        "0x65eb6490da70f68ab2165a98155948ecb8188a46",
        ERC20_ABI,
        provider,
      );
